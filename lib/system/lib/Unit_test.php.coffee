  ...
  result : ($results = {}) ->
    ...
    for $result in $results
      ...
      for $key, $val of $result
        ...
        if is_array($val)
          for $k, $v of $val
            if false isnt ($line = @i18n.line(strtolower('ut_' + $v)))
              $v = $line
              
            $temp[@i18n.line('ut_' + $k)] = $v

        else 
          if false isnt ($line = @i18n.line(strtolower('ut_' + $val)))
            $val = $line
...
module.exports = ExspressoUnit_test
#  END Unit_test Class



#  End of file Unit_test.php 
#  Location: ./system/lib/Unit_test.php