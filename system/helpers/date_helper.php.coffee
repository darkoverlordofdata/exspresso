...
{config, date, defined, explode, floor, function_exists, get_instance, gmdate, is_numeric, item, lang, line, load, mktime, preg_match, preg_replace, str_replace, strlen, strtolower, substr, time, trim}  = require(FCPATH + 'helper')
...
if not function_exists('mdate')
  exports.mdate = mdate = ($datestr = '', $time = '') ->
    if $datestr is '' then return ''if $time is '' then $time = now()$datestr = str_replace('%\\', '', preg_replace("/([a-z]+?){1}/i", "\\\\\\1", $datestr))return date($datestr, $time)}}if not function_exists('standard_date')
      ...
      exports.timespan = timespan = ($seconds = 1, $time = '') ->
        ...
        if $years > 0
          $str+=$years + ' ' + $CI.lang.line((($years > 1) then 'date_years' else 'date_year')) + ', '
        ...
        if $years > 0 or $months > 0
          if $months > 0
            $str+=$months + ' ' + $CI.lang.line((($months > 1) then 'date_months' else 'date_month')) + ', '
        ...
        if $years > 0 or $months > 0 or $weeks > 0
          if $weeks > 0
            $str+=$weeks + ' ' + $CI.lang.line((($weeks > 1) then 'date_weeks' else 'date_week')) + ', '
        ...
        if $months > 0 or $weeks > 0 or $days > 0
          if $days > 0
            $str+=$days + ' ' + $CI.lang.line((($days > 1) then 'date_days' else 'date_day')) + ', '
        ...
        if $days > 0 or $hours > 0
          if $hours > 0
            $str+=$hours + ' ' + $CI.lang.line((($hours > 1) then 'date_hours' else 'date_hour')) + ', '
        ...
        if $days > 0 or $hours > 0 or $minutes > 0
          if $minutes > 0
            $str+=$minutes + ' ' + $CI.lang.line((($minutes > 1) then 'date_minutes' else 'date_minute')) + ', '
        ...
        if $str is ''
          $str+=$seconds + ' ' + $CI.lang.line((($seconds > 1) then 'date_seconds' else 'date_second')) + ', '
      ...
      exports.local_to_gmt = local_to_gmt = ($time = '') ->
        if $time is '' then $time = time()return mktime(gmdate("H", $time), gmdate("i", $time), gmdate("s", $time), gmdate("m", $time), gmdate("d", $time), gmdate("Y", $time))}}if not function_exists('gmt_to_local')
          ...
          exports.human_to_unix = human_to_unix = ($datestr = '') ->
            ...
            if $split['2']? 
              ...
              if substr($ampm, 0, 1) is 'p' and $hour < 12 then $hour = $hour + 12if substr($ampm, 0, 1) is 'a' and $hour is 12 then $hour = '00'if strlen($hour) is 1 then $hour = '0' + $hour}return mktime($hour, $min, $sec, $month, $day, $year)}}if not function_exists('timezone_menu')
                exports.timezone_menu = timezone_menu = ($default = 'UTC', $class = "", $name = 'timezones') ->
                  $CI = get_instance()
                  $CI.lang.load('date')
                  
                  if $default is 'GMT' then $default = 'UTC'$menu = '<select name="' + $name + '"'if $class isnt ''
                    $menu+=' class="' + $class + '"'
                    $menu+=">\n"
                ...
                exports.timezones = timezones = ($tz = '') ->
                  ...
                  if $tz is 'GMT' then $tz = 'UTC'return if ( not $zones[$tz]? ) then 0 else $zones[$tz]}}#  End of file date_helper.php #  Location: ./system/helpers/date_helper.php #  ------------------------------------------------------------------------#
              # Timezone Menu
              #
