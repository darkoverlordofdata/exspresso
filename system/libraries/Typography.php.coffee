...
{array_keys, count, defined, explode, in_array, nl2br, preg_match, preg_match_all, preg_replace, preg_replace_callback, preg_split, rtrim, str_replace, strpos}	= require(FCPATH + 'helper')
...
class CI_Typography
	...
	auto_typography : ($str, $reduce_linebreaks = false) ->
		...
		if strpos($str, '<!--') isnt false
			if preg_match_all("#(<!\-\-.*?\-\->)#s", $str, $matches)
				($i = 0,$total = count($matches[0])$i < $total$i++)
				{
		...
		($i = 0,$total = count($html_comments)$i < $total$i++)
		{
	...
	nl2br_except_pre : ($str) ->
		...
		if $ct - 1 isnt $i then $newstr+="pre>"
		}
...
register_class 'CI_Typography', CI_Typography
module.exports = CI_Typography
