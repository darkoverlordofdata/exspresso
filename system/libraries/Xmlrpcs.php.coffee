...
class CI_Xmlrpcs extends CI_Xmlrpc
	...
	parseRequest : ($data = '') ->
		...
		if not xml_parse($parser, $data, 1)
			...
			($i = 0$i < count($parser_object.xh[$parser]['params'])$i++)
			{
			if @debug is true
...
# @return	object
#
exports.listMethods = listMethods = ($m) ->
...
# @return	object
#
exports.methodSignature = methodSignature = ($m) ->
...
# @return	object
#
exports.methodHelp = methodHelp = ($m) ->
	$parameters = $m.output_parameters()
	$method_name = $parameters[0]
...
# @return	object
#
exports.multicall_error = multicall_error = ($err) ->
	$str = if is_string($err) then @xmlrpcstr[multicall_${err}] else $err.faultString()$code = if is_string($err) then @xmlrpcerr[multicall_${err}] else $err.faultCode()
...
# @return	object
#
exports.do_multicall = do_multicall = ($call) ->
